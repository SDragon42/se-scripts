...
namespace IngameScript
{
    partial class Program : MyGridProgram
    {
        ...
        readonly List<IMyTerminalBlock> _maintCarrageRamps = new List<IMyTerminalBlock>();
        ...
        public void Main(string argument)
        {
            try
            {
                Echo("Maint. Carriage Control " + VERSION + ": " + _runSymbol.GetSymbol(this.Runtime));
        ...
        void LoadBlockLists(bool forceLoad = false)
        {
            ...
            GridTerminalSystem.GetBlocksOfType<IMyMotorStator>(_maintCarrageRamps, IsTaggedBlockOnThisGrid);
        ...
        void RaiseRamps()
        {
            ...
            Common.ExecuteForAll(
                _maintCarrageRamps,
                rotor => _rampsClear &= Rotate2Limit(rotor, false));
        ...
        bool IsRotated2Limit(IMyTerminalBlock b, bool rotateToMax)
        {
            var rotor = b as IMyMotorStator;
            if (rotor == null) return true;
        ...
        void CheckRampsAtLimits()
        {
            ...
            Common.ExecuteForAll(
                _maintCarrageRamps,
                rotor => allRaised &= IsRotated2Limit(rotor, false));
            Common.ExecuteForAll(
                _maintCarrageRamps,
                rotor => allLowered &= IsRotated2Limit(rotor, true));
            if (!(allRaised || allLowered)) return;
            Common.ExecuteForAll(_maintCarrageRamps, b => ((IMyMotorStator)b).SafetyLock = true);
